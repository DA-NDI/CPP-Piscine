...
//   Updated: 2018/06/26 16:49:09 by avolgin          ###   ########.fr       //
//                                                                            //
// ************************************************************************** //
...
void Form::execute(Bureaucrat const & executor) const throw(Form::GradeTooLowException){
	if (!this->getStatus()) {
	std::cout << executor.getName() << " can't execute "\
	<< this->getName() << " form. It is not signed." << std::endl;
	return ;
	}
