...
//   Updated: 2018/06/21 22:42:59 by avolgin          ###   ########.fr       //
//                                                                            //
// ************************************************************************** //
...
Fixed		Fixed::operator*(Fixed const & rhs) const{
	// std::cout << "this->getRawBits()" << this->getRawBits() << std::endl;
	// std::cout << "rhs.getRawBits()" << rhs.getRawBits() << std::endl;

	// float i = this->getRawBits() * rhs.getRawBits();
	// std::cout << i << std::endl;
	// if (i & 0xFF)
	// 	return Fixed((float)(i >> 8));
	// else
	// 	return Fixed(i >> 8);
	if (this->getRawBits() & 0xFF && rhs.getRawBits() & 0xFF)
		return Fixed(this->toFloat() * rhs.toFloat());
